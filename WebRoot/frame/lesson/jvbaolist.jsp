...
<script type="text/javascript">
	function fanye(page){
	top.window.frmright.location="reportInformationslist.action?nowPage="+page;
	}
	function shanchu(id){
	  //alert(id);
	  ...
	  //ajax删除一条评论信息
	  $.post(
	   "delReportInformationflow.action",
	   {"rid":id},
	   function(data){
	     if(data.ok==1){
	  ...
	  if(""==idsString){
	   top.Dialog.alert("请选择删除项！");
	    ...
	    top.Dialog.confirm("确定要删除吗？",function(){
	     //ajax批量删除
	       $.post(
	         "delManyReportInformationflow.action",
	         {"ids":idsString},
	         function(data){
	           if(data.ok==1){
	             window.location.href=window.location.href;
	...
	function wuxiao(id){
	top.Dialog.confirm("确定要设置为无效吗？",function(){
	  //ajax删除一条评论信息
	  $.post(
	   "settReportInformationState.action",
	   {"rid":id},
	   function(data){
	     if(data.ok==1){
	      window.location.href=window.location.href;
...
<div>
	<table class="tableStyle" sortMode="true" headFixMode="true" useMultColor="true" useCheckBox="true">
		<tr>
			<th width="25"></th>
			<th width="100">信息流ID</th>
			<th width="100">发布者昵称</th>
			<th width="100">举报人ID</th>
			<th width="100">举报人昵称</th>
			<th>图片缩略图</th>
...
<div id="scrollContent" >
	<table class="tableStyle"  useMultColor="true" useCheckBox="true">
	   ...
	   <s:set value="#status.index" id="idx"/>
		<tr>
			<td width="25"><input type="checkbox" name="ids" value="${list[idx].id}"/></td>
			<td width="100"><s:property value="list[#idx].informationflow.id"/></td>
			<td width="100"><s:property value="list[#idx].informationflow.user.nickName"/></td>
			<td width="100"><s:property value="list[#idx].user.id"/></td>
			<td width="100"><s:property value="list[#idx].user.nickName"/></td>
			<td>
			<s:iterator value="slts[#idx]" id="str">
			<img src="${str }" width="30px;" height="30px;"/>
			</s:iterator>
			</td>
			<td width="100"><s:property value="list[#idx].time"/></td>
			<td width="80"><span class="img_delete hand" title="删除" onclick="shanchu('${list[idx].id}')"></span>
			<s:if test="list[#idx].state==2">已无效</s:if>
			<s:else>
			<input type="button" value="无效" onclick="wuxiao('${list[idx].id}')" class="default"/>
			</s:else>
			</td>
		</tr>
	</s:iterator>	
