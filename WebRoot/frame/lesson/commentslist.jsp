...
<div>
	<table class="tableStyle" sortMode="true" headFixMode="true" useMultColor="true" useCheckBox="true">
		<tr>
			...
			<th width="100">评论者ID</th>
			<th width="100">昵称</th>
			<th width="100"><span onclick="top.Dialog.alert('排序的功能要在服务器端完成。详情请参照基本表格一节关于排序的说明')">评论时间</span></th>
			<th>评论内容</th>
			<th width="80">操作</th>
		</tr>
	</table>
...
<div id="scrollContent" >
	...
	<s:iterator value="list" id="comment">
		<tr>
			<td width="25"><input type="checkbox" name="ids" value="${comment.id }"/></td>
			<td width="100">${comment.informationflow.id }</td>
			<td width="100">${comment.userStart.id }</td>
			<td width="100">
				${comment.userStart.nickName }
			</td>
			<td width="100">${comment.time }</td>
			<td>
				<span class="text_slice" style="width:1000px;" title="${comment.content}">${comment.content}</span>
			</td>
			<td width="80"><span class="img_delete hand" title="删除" onclick="shanchu('${comment.id }')"></span></td>
		</tr>
	</s:iterator>	
	</table>
</div>
